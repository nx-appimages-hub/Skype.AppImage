...
all:
	...
	mkdir --parents $(PWD)/build/AppDir/skypeforlinux	
	cp --force --recursive $(PWD)/build/usr/* $(PWD)/build/AppDir/
	cp --force --recursive $(PWD)/build/usr/share/skypeforlinux/* $(PWD)/build/AppDir/skypeforlinux
	cp --force --recursive $(PWD)/AppDir/* $(PWD)/build/AppDir

	rm -rf $(PWD)/AppDir/opt
	rm -rf $(PWD)/build/usr/share/skypeforlinux

	chmod +x $(PWD)/build/AppDir/AppRun
	chmod +x $(PWD)/build/AppDir/*.desktop

	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/AppDir $(PWD)/Skype.AppImage
